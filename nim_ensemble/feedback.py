...
def resolve_feedback(
    ...
    if confirmed:
        ...
        elo.update_from_override(votes, entry["answer"])
        entry["resolved"] = True
        entry["feedback"] = "confirmed"
    elif correct_answer:
        # User says the correct answer was something else
        elo.update_from_override(votes, correct_answer.upper())
        entry["resolved"] = True
        entry["feedback"] = f"override:{correct_answer.upper()}"
    else:
...
def stats() -> dict:
    ...
    overridden = sum(1 for e in entries if e.get("feedback", "").startswith("override:"))
